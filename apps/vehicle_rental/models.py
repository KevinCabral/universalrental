...
class CustomerNotification(models.Model):
    ...
    TYPE_CHOICES = [
        ...
        ('rental_cancellation', _('Cancelamento de Aluguer')),
        ('rental_reminder', _('Lembrete de Aluguer')),
        ('payment_confirmation', _('Confirmação de Pagamento')),
        ('other', _('Outro')),
    ]
    ...
    customer = models.ForeignKey(
        ...
        help_text=_("Cliente destinatário da notificação")
    )
    rental = models.ForeignKey(
        Rental,
        on_delete=models.SET_NULL,
        ...
        blank=True,
        related_name='notifications',
        help_text=_("Aluguer relacionado (se aplicável)")
    )
    ...
    notification_type = models.CharField(
        ...
        default='other',
        help_text=_("Tipo de notificação")
    )
    recipient_email = models.EmailField(
        help_text=_("Email do destinatário")
    )
    ...
    content = models.TextField(
        help_text=_("Conteúdo em texto simples")
    )
    html_content = models.TextField(
        blank=True,
        null=True,
        help_text=_("Conteúdo HTML do email")
    )
    ...
    status = models.CharField(
        ...
        default='pending',
        help_text=_("Estado do envio")
    )
    error_message = models.TextField(
        blank=True,
        null=True,
        help_text=_("Mensagem de erro (se falhou)")
    )
    attempt_count = models.PositiveIntegerField(
        default=0,
        help_text=_("Número de tentativas de envio")
    )
    ...
    sent_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text=_("Data de envio bem-sucedido")
    )
    created_by = models.ForeignKey(
        User,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        help_text=_("Utilizador que criou/reenviou")
    )
    ...
    def __str__(self):
        return f"{self.get_notification_type_display()} - {self.customer.full_name} ({self.get_status_display()})"
    
    def send(self, user=None):
        """
        Send or resend the notification email.
        ...
        try:
            ...
            logger.info(f'Notification #{self.id} sent successfully to {self.recipient_email}')
            return True, None
            ...
            logger.exception(f'Notification #{self.id} failed: {error_msg}')
